				...
				function wooco_show_items( $product = null ) {
					...
					if ( $components = $product->get_components() ) {
                        ...
                        <div class="wooco_components wooco-components"
							...
							<?php
							foreach ( $components as $component ) {
								if ( ( ( $component_type = $component['type'] ) === '' ) || empty( $component[ $component_type ] ) ) {
									continue;
								}

								$component_class      = 'wooco_component wooco_component_' . $count . ' wooco_component_type_' . $component_type;
								$component_default    = isset( $component['default'] ) ? (int) $component['default'] : 0;
								$component_default    = isset( $df_products[ $count - 1 ] ) ? absint( $df_products[ $count - 1 ] ) : $component_default;
								...
								$component_products   = $this->wooco_get_products( $component_type, $component_orderby, $component_order, $component[ $component_type ], $component_exclude, $component_default, $component_qty, $component_price );

								if ( ! $component_products && ( $component['optional'] === 'yes' ) ) {
									// if have no products and isn't required, hide it
									continue;
								}
								...
								do_action( 'wooco_before_component', $component, $count );
								...
								if ( ! $component_products ) {
									if ( $component['optional'] !== 'yes' ) {
                                        ...
                                        <div class="wooco_component_product wooco_component_product_none"
                                             ...
                                             data-required="<?php echo esc_attr( $component['optional'] !== 'yes' ? 'yes' : 'no' ); ?>"></div><!-- /wooco_component_product -->
										<?php
									}
									...
									if ( $only_product ) {
										$only_product_id   = $only['id'];
										$only_product_name = '';

										if ( $only_product->is_visible() && ( get_option( '_wooco_product_link', 'yes' ) !== 'no' ) ) {
											$only_product_name .= '<a ' . ( get_option( '_wooco_product_link', 'yes' ) === 'yes_popup' ? 'class="woosq-btn" data-id="' . $only_product_id . '"' : '' ) . ' href="' . get_permalink( $only_product_id ) . '" ' . ( get_option( '_wooco_product_link', 'yes' ) === 'yes_blank' ? 'target="_blank"' : '' ) . '>';
										}

										if ( $only['purchasable'] !== 'yes' ) {
											$only_product_name .= '<s>';
										}

										$only_product_name .= ( $component['custom_qty'] !== 'yes' && $show_qty === 'yes' ? $component['qty'] . ' &times; ' . $only['name'] : $only['name'] );
                                        ...
                                        <div class="wooco_component_product wooco_component_product_only"
											...
											<?php if ( get_option( '_wooco_checkbox', 'no' ) === 'yes' ) { ?>
                                                <div class="wooco_component_product_checkbox">
                                                    <input class="wooco-checkbox" type="checkbox" checked="checked"
                                                           value="40" <?php echo( $component['optional'] !== 'yes' ? 'disabled' : '' ); ?>/>
                                                </div>
											<?php } ?>
											...
											<?php if ( $show_price === 'yes' ) { ?>
                                                <div class="wooco_component_product_price">
													<?php echo wc_price( $only['price'] ); ?>
                                                </div>
											<?php } ?>

											<?php if ( $component['custom_qty'] === 'yes' ) {
												$_min = 0;
												$_max = 1000;
                                    ...
                                    <div class="wooco_component_product"
										...
										<?php if ( ( $selector === 'select' ) && ( $show_image === 'yes' ) ) { ?>
                                            <div class="wooco_component_product_image">
                                        ...
                                        <div class="wooco_component_product_selection">
                                            <select class="wooco_component_product_select"
                                                    id="<?php echo esc_attr( 'wooco_component_product_select_' . $count ); ?>">
												<?php
												$option_none = get_option( '_wooco_option_none', '' );

												if ( empty( $option_none ) ) {
													$option_none = esc_html__( 'No, thanks. I don\'t need this', 'wpc-composite-products' );
												}

												if ( ( get_option( '_wooco_checkbox', 'no' ) === 'no' ) && ( ( $component['optional'] === 'yes' ) || ( get_option( '_wooco_option_none_required', 'no' ) === 'yes' ) ) ) {
													echo '<option value="-1" data-pid="-1" data-qty="0" data-price="" data-link="" data-price-html="" data-imagesrc="' . esc_url( apply_filters( 'wooco_option_none_img_src', wc_placeholder_img_src() ) ) . '" data-imagefull="' . esc_url( apply_filters( 'wooco_option_none_img_full', wc_placeholder_img_src() ) ) . '" data-description="' . esc_attr( apply_filters( 'wooco_option_none_description', htmlentities( wc_price( 0 ) ) ) ) . '">' . esc_html( $option_none ) . '</option>';
												}

												foreach ( $component_products as $component_product ) {
													echo '<option value="' . esc_attr( $component_product['purchasable'] === 'yes' ? $component_product['id'] : 0 ) . '" data-pid="' . esc_attr( $component_product['pid'] ) . '" data-price="' . esc_attr( $component_product['price'] ) . '" data-link="' . esc_url( $component_product['link'] ) . '" data-price-html="' . esc_attr( $component_product['price_html'] ) . '" data-imagesrc="' . esc_url( $component_product['image'] ) . '" data-imagefull="' . esc_url( $component_product['image_full'] ) . '" data-description="' . esc_attr( $component_product['description'] ) . '" ' . ( $component_product['id'] == $component['default'] ? 'selected' : '' ) . '>' . ( $component['custom_qty'] !== 'yes' && $show_qty === 'yes' ? $component['qty'] . ' &times; ' . $component_product['name'] : $component_product['name'] ) . '</option>';
												}
												?>
                                            </select>
                                        </div>

										<?php if ( ( $selector === 'select' ) && ( $show_price === 'yes' ) ) { ?>
								...
								do_action( 'wooco_after_component', $component, $count );
								echo '</div><!-- /wooco_component -->';
								$count ++;
							} ?>
